...
def create_RepVGG_A0g4(deploy=False):
    return RepVGG(num_blocks=[2, 4, 14, 1], num_classes=1000,
                  width_multiplier=[0.75, 0.75, 0.75, 2.5], override_groups_map=g4_map, deploy=deploy)

def create_RepVGG_A0g8(deploy=False):
    return RepVGG(num_blocks=[2, 4, 14, 1], num_classes=1000,
                  width_multiplier=[0.75, 0.75, 0.75, 2.5], override_groups_map=g8_map, deploy=deploy)

def create_RepVGG_A0g16(deploy=False):
    return RepVGG(num_blocks=[2, 4, 14, 1], num_classes=1000,
                  width_multiplier=[0.75, 0.75, 0.75, 2.5], override_groups_map=g16_map, deploy=deploy)
...
def whole_model_convert(train_model:torch.nn.Module, deploy_model:torch.nn.Module, save_path=None):
    ...
    for name, module in train_model.named_modules():
        if (hasattr(module, 'preupsample')) or (name == 'encoder'):
            continue
...
def visual_weight(all_weights):
    for name, weiight in all_weights.items():
        print(name)
